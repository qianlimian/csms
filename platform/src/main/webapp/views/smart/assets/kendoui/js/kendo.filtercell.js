/** 
 @description smart框架覆盖grid列过滤源码，主要针对foreignkey column
        ...
        var FilterCell = Widget.extend({
            init: function (element, options) {
                ...
                if (options.values) {
                    ...
                    //smart框架修改，转换枚举类型的options.type
                    type = ENUM;
                    if (options.type == "enum") {
                        options.type = options.values[0]["className"];
                    }
                }
            ...
            _refreshUI: function () {
                ...
                //smart框架修改
                var that = this, filter = findFilterForField(that.dataSource.filter(), this.options.dataTextField || this.options.field) || {}, viewModel = that.viewModel;
            ...
            updateDsFilter: function (e) {
                ...
                //smart框架修改，filter增加属性type
                var currentFilter = $.extend({}, that.viewModel.toJSON(), { field: that.options.dataTextField || that.options.field, type: that.options.type });
                var expression = {
                    logic: 'and',
                    filters: []
                };
            ...
            _merge: function (expression) {
                ...
                //smart框架修改
                removeFiltersForField(result, that.options.dataTextField || that.options.field);
