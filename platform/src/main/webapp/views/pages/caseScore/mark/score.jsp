...
<div id="ctnCaseScore">
    ...
    <h6 style="float:left;height:26px;line-height:26px;">
        <span>案件名称：<span style="text-decoration:underline;font-weight:700;margin-right:20px">${score.caseName}</span></span>
        <span>案件得分：<span style="text-decoration:underline;font-weight:700;" id="score_totalScore">${score.totalScore}分</span></span>
    </h6>
    <div class="s-toolbar">
    ...
    <table class="table table-bordered">
        ...
        <c:forEach items="${bigItems}" var="bigItem">
            <tr>
            <th rowspan="${bigItem.treeleafs.size()+1}" colspan="2"
                scope="row">${bigItem.seq }、${bigItem.standard }（${bigItem.score } 分）
            </th>
                                ...
                                <c:forEach items="${score.itemDtos }" var="itemSelected">
                                    <c:choose>
                                        <c:when test="${itemSelected.evalId eq detail.id }">
                                            <option value="${detail.score }" id="${detail.id }"
                                                    selected>${detail.standard }</option>
